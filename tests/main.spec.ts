// Source: https://github.com/amannn/next-intl/blob/main/examples/example-app-router/tests/main.spec.ts
// CURRENTLY MOSTLY FAILING TESTS
...
it("sets caching headers", async ({ request }) => {
	for (const pathname of ["/en", "/en/pathnames", "/de", "/de/pfadnamen"]) {
		expect(
			(await request.get(pathname)).headers()["cache-control"],
...
it("serves a robots.txt", async ({ page }) => {
	...
	expect(body?.toString()).toEqual("User-Agent: *\Disallow: /\n");
});
